...
def infos_livre(url_product_page):
    ...
    # Pour trouver le nom de la catégorie, on extrait la barre de navigation horizontale du dessus .
    barre_de_navigation = soup.find("ul", class_="breadcrumb")
    nom_category = barre_de_navigation.find_all("li")[-2].a.text
    ...
    infos_livres = [upc, title, price_excl_tax, price_incl_tax, availability, nom_category,
                    number_of_reviews, url_image, etoiles, product_description]

    # APPEL DE LA FONCTION "ECRIRE DANS DES FICHIERS CSV"
    ecrire_dans_csv(nom_category, infos_livres)

    # APPEL DE LA FONCTION "SAUVEGARDER DES IMAGES"
    sauvegarder_images(url_image, nom_category, nom_image)


def ecrire_dans_csv(nom_category, infos_livres):
    ...
    filename = os.path.join(
        os.getcwd(), f"csv\{nom_category}.csv")  # creation de fichier csv
    ...
    if not os.path.exists(filename):
        with open(filename, "w", newline="", encoding="utf8") as file:
            ...
            # Ecriture de l'en-tête de chaque fichier csv.
            writer.writerow(label_colonnes)
            # Ecriture des infos dans chaque fichier csv.
...
def sauvegarder_images(url_image, nom_category, nom_image):
    # Téléchargement et sauvegarde des images.
    # Cette fonction est appelée par la fonction "infos_livre".
    response = requests.get(url_image)

    # créer le dossier s'il est inexistant.
    if not os.path.exists(f"images\{nom_category}"):
        os.makedirs(f"images\{nom_category}")

    filename = os.path.join(
        os.getcwd(), f"images\{nom_category}\{nom_image}.jpg")  # Création fichier image
